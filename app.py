...
def adjust_risk(base_risk, rainfall_mm, recent_rain_level, humidity):
    ...
    humidity_factor = max(0, (humidity - 60) / 40) * 0.2
...
if st.button("🔥 화재 위험도 예측", type="primary"):
    ...
    try:
        ...
        base_risk = proba * 100
        adjusted_risk, moisture_factor = adjust_risk(
        ...
        if reduction > 0:
            ...
            st.warning(f"⚠️ 건조 효과로 위험도 {abs(reduction):.1f}%p 증가")
        else:
            st.info("💧 습윤 효과 없음")
