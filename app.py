...
with col1:
    기온 = st.number_input("기온 (°C)", value=0.0, step=1.0, key="temp")
    풍속 = st.number_input("풍속 (m/s)", value=0.0, step=1.0, key="wind_speed")
    이슬점온도 = st.number_input("이슬점온도 (°C)", value=0.0, step=1.0, key="dew_point")
    월 = st.selectbox("월", list(range(1,13)), index=0, key="month") 
with col2:
    강수량 = st.number_input("현재 강수량 (mm)", value=0.0, step=1.0, min_value=0.0, key="rainfall")
    습도 = st.number_input("습도 (%)", value=0.0, step=1.0, min_value=0.0, max_value=100.0, key="humidity")
    기압 = st.number_input("기압 (hPa)", value=0.0, step=1.0, key="pressure")
    시간 = st.selectbox("시간", list(range(24)), index=0, key="hour")
with col3:
    ...
    format_func=lambda x: {
        0: "☀️ 매우 건조 - 최근 3일간 비나 눈 없음 (바닥이 완전히 말라있음)",
        1: "🌤️ 건조 - 2-3일 전 약간의 비/눈 (바닥이 거의 마른 상태)",
        2: "⛅ 보통 - 1-2일 전 비/눈 (바닥이 조금 젖어있음)",
        3: "🌧️ 습윤 - 24시간 내 비/눈 (바닥에 물기가 있음)",
        4: "❄️ 매우 습윤 - 최근 많은 비/눈 (곳곳에 물웅덩이/잔설 존재)",
        5: "💧 포화 - 지금도 비/눈이 오거나, 땅이 완전히 젖어있음 (진흙/물이 고여있음)"
    }[x],
    index=1,
    key="rain_level"
)
